...
router.post('/place', auth, async (req: AuthRequest, res) => {
  ...
  if (stake < 1) return res.status(400).json({ error: 'Minimum stake is 1 Lëk' });
...
router.post('/cashout/:ticketId', auth, async (req: AuthRequest, res) => {
  ...
  const ticket = await prisma.ticket.findUnique({ where: { id: ticketId, userId: req.user!.userId } });
